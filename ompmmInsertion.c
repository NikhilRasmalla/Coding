...
void minMaxInsertion(double **distMatrix, int numOfCoords, int *tour) {
    ...
    while (tourLength < numOfCoords) { // Continue until all nodes are visited
        ...
        #pragma omp parallel // Start a parallel region
        {
            ...
            double localMinMaxDist = DBL_MAX; // Thread-local variable for the min-max distance
            ...
            for (int i = 0; i < numOfCoords; i++) { // Loop over all nodes
                if (!visited[i]) { // Consider only unvisited nodes
                    ...
                    for (int j = 0; j < tourLength; j++) {
                        if (distMatrix[tour[j]][i] > maxDist) {
                            maxDist = distMatrix[tour[j]][i]; // Update maxDist if a larger distance is found
                        }
                    }
                    if (maxDist < localMinMaxDist) { // Check if this maxDist is the minimal max distance found
                        localMinMaxDist = maxDist;
            ...
            {
                if (localMinMaxDist < minMaxDist) { // Check if this thread found a new global minimum
                    minMaxDist = localMinMaxDist;
